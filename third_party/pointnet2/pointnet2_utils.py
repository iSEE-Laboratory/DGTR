...
def knn(ref: Tensor, query: Tensor, k: int = 1):
    """
...
class QueryAndGroup(nn.Module):
    r"""
    Groups with a ball query of radius or knn

    Parameters
    ---------
    radius : float32 or None (if radius is None, use knn query)
        Radius of ball
    nsample : int32
        Maximum number of features to gather in the ball or "k" in knn
    """
    ...
    def __init__(self, radius, nsample, use_xyz=True, ret_grouped_xyz=False, normalize_xyz=False, sample_uniformly=False, ret_unique_cnt=False):
        ...
        if radius is None:
            print("Using KNN as the query operator of Encoder")
            assert not normalize_xyz, "Cannot normalize xyz with radius=None"
    ...
    def forward(self, xyz, new_xyz, features=None):
        ...
        if self.radius is not None:
            ...
            ref = xyz.transpose(-1, -2).contiguous()
            query = new_xyz.transpose(-1, -2).contiguous()
            idx = knn(ref, query, self.nsample).transpose(-1, -2).contiguous().int()  # (B, N, nsample)
